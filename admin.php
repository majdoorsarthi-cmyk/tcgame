...
<body>
    ...
    <div class="login-card">
        <div class="brand">⚡ TC MASTER ENGINE</div>
...
if (isset($_GET['action_w']) && isset($_GET['wid'])) {
    ...
    if ($w_row = mysqli_fetch_assoc($w_res)) {
        if ($act_w === 'approve' && $w_row['status'] == 'Pending') {
            ...
            // Auto Refund back to user's wallet balance
    ...
    <div class="sidebar">
        <div>
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item <?= $active_tab == 'users' ? 'active' : '' ?>"><a href="admin.php?tab=users">👥 User Control</a></li>
                <li class="nav-item <?= $active_tab == 'withdrawals' ? 'active' : '' ?>"><a href="admin.php?tab=withdrawals">💳 विथड्रॉल (<?= $pending_w ?>)</a></li>
                <li class="nav-item <?= $active_tab == 'settings' ? 'active' : '' ?>"><a href="admin.php?tab=settings">⚙️ System Settings</a></li>
            </ul>
        </div>
        <div class="user-profile-badge">
            <div>
                <div style="font-size:12px; font-weight:700; color:#fff;"><?= htmlspecialchars($_SESSION['admin_user']) ?></div>
                <div style="font-size:10px; color:var(--text-muted);">Root Admin</div>
            </div>
